...
Page({
  ...
  loadRandomGroups() {
    ...
    // 为每个组添加选择状态与补全组ID
    const groupsWithSelection = dailyRandom.map((group: any) => {
      ...
      const tasksWithId = tasks.map((t: any) => ({
        ...t,
        displayId: this.computeTaskDisplayId('daily', t)
      }))
      return {
      ...group,
        id: gid || group.id,
  ...
  deleteTask(e: any) {
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          const pools = wx.getStorageSync('questPools') || {}
          const group = pools.daily.random.groups.find((g: any) => g.name === groupName)
  ...
  deleteGroup(e: any) {
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          const pools = wx.getStorageSync('questPools') || {}
          pools.daily.random.groups = pools.daily.random.groups.filter((g: any) => g.name !== groupName)
          wx.setStorageSync('questPools', pools)
          this.loadRandomGroups()
  ...
  addToGroup() {
    ...
    const pools = wx.getStorageSync('questPools') || {}
    if (!pools.daily || !pools.daily.random) {
    ...
    if (matchedGroup) {
      ...
      // 查找同名组
      targetGroup = pools.daily.random.groups.find((g: any) => g.name === finalGroupName)
      ...
      if (!targetGroup) {
        ...
        pools.daily.random.groups.push(targetGroup)
      }
    ...
    selectedTasks.forEach((task: any) => {
      ...
      if (!existingTask) {
        ...
        // 从原组中移除任务
        const sourceGroupObj = pools.daily.random.groups.find((g: any) => g.name === task.sourceGroup)
        if (sourceGroupObj) {
  ...
  updateTask() {
    ...
    const pools = wx.getStorageSync('questPools') || {}
    const group = pools.daily.random.groups.find((g: any) => g.name === editingGroup)
